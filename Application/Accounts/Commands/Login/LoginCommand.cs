...
{
    public class LoginCommand : IRequest<ResultSingle<UserDto>>
    {
    ...
    public class LoginCommandHandler : IRequestHandler<LoginCommand, ResultSingle<UserDto>>
    {
        ...
        public LoginCommandHandler(UserManager<AppUser> userManager, IOptions<JwtSettings> jwtSettings, IMapper mapper, MeetingDbContext dbContext)
        {
        ...
        public async Task<ResultSingle<UserDto>> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            ...
            if (appUser == null)
            {
                return Result.Fail<UserDto>(null, $"{request.Email} adresi kayıtlı değil");
            }
            ...
            return Result.Fail<UserDto>(null, $"{request.Email} mail adresi veya şifre yanlış.");
